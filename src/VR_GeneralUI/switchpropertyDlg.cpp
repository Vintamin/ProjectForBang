#include "vrmine/vr_generalui/switchpropertyDlg.h"
#include "ui_switchpropertyDlg.h"
...
namespace VirtualMine
{
	namespace GeneralUI
	{
		CSwitchPropertyDlg::CSwitchPropertyDlg(QWidget *parent /* = 0 */)
			:QDialog(parent)
		{
			setWindowFlags(Qt::CustomizeWindowHint | Qt::WindowCloseButtonHint |Qt::Tool | Qt::X11BypassWindowManagerHint);
			setFixedSize(this->width(), this->height());
			this->ui = new Ui::SwitchDlg();
			this->ui->setupUi(this);
		}

		CSwitchPropertyDlg::~CSwitchPropertyDlg()
		{
			setAttribute (Qt::WA_DeleteOnClose);
		}

		void CSwitchPropertyDlg::setID(const std::string& substationID,const std::string& sensorID)
		{
			this->mSubstationID = substationID;
			this->mSensorID = sensorID;
		}

		void CSwitchPropertyDlg::initialize(QDomDocument* doc)
		{
			if (!doc)
			{
				return;
					...
					if (child.toElement().tagName() == "data")
					{
						...
						{					
							...
							if (defaultID == QString::fromStdString(this->mSubstationID))
							{
								...
								//this->ui->baojingLabel->setText(child.toElement().attribute("DevName"));
								this->ui->statusLabel->setText(child.toElement().attribute("CurrValue") + QString(" / ") + child.toElement().attribute("CurrState"));
								this->ui->baojingStautsLabel->setText(child.toElement().attribute("AlertState") + QString(" / ") + child.toElement().attribute("AlertTime"));
								//this->ui->nameLabel->setText(child.toElement().attribute("DevName"));
