#include "vrmine/vr_generalui/basedialog.h"
...
CUiBaseDialog::CUiBaseDialog(QWidget *parent)
	: QDialog(parent)
{
	mMoveing			= false;
	mLabelIcon			= NULL;
...
void CUiBaseDialog::resizeEvent( QResizeEvent *event )
{
...
void CUiBaseDialog::mousePressEvent( QMouseEvent *event )
{
	mMoveing = true;
...
void CUiBaseDialog::mouseMoveEvent( QMouseEvent *event )
{
...
void CUiBaseDialog::mouseReleaseEvent(QMouseEvent *event)
{
	mMoveing = false;
}

void CUiBaseDialog::createTitleBar(QWidget *parent,uint flag,QString title /* = "" */)
{
	if (parent == NULL)
		return;
	...
	if ((flag & WindowToolIcon) == WindowToolIcon)
	{
		...
		mLabelIcon->setPixmap(QPixmap(":/workstation/Resources/systembutton/logo.ico").scaled(16,16));
...
void CUiBaseDialog::doToolButtonMax()
{
...
void CUiBaseDialog::doToolButtonNor()
{
...
void CUiBaseDialog::doToolButtonClose()
{
	close();
}